...
class DailyContentCog(commands.Cog):
    ...
    def start_schedulers(self):
        """全スケジューラーを開始"""
        if not self.entrepreneur_quotes.is_running():
            self.entrepreneur_quotes.start()
        if not self.skill_tips.is_running():
            self.skill_tips.start()
        if not self.daily_challenge.is_running():
            self.daily_challenge.start()
        if not self.business_trends.is_running():
            self.business_trends.start()
    
    def cog_unload(self):
        """Cogがアンロードされる時にタスクを停止"""
        self.entrepreneur_quotes.cancel()
        self.skill_tips.cancel()
        self.daily_challenge.cancel()
        self.business_trends.cancel()
    ...
    async def on_ready(self):
        ...
        self.start_schedulers()
        print("Daily content schedulers started")
    
    # 1. 起業家格言bot（設定に基づく時刻）
    @tasks.loop(minutes=1)  # 1分ごとにチェック
    async def entrepreneur_quotes(self):
        """起業家格言を設定時刻に配信"""
        ...
        for guild in self.bot.guilds:
            ...
            if (settings.quotes_enabled and 
                ...
                now.minute == settings.quotes_minute):
    ...
    async def skill_tips(self):
        ...
        await self.send_to_all_guilds(embed, "tips")
    
    # 3. 今日のチャレンジ（毎日朝10時）
    @tasks.loop(time=datetime.time(hour=10, minute=0, tzinfo=datetime.timezone(datetime.timedelta(hours=9))))
    async def daily_challenge(self):
        ...
        await self.send_to_all_guilds(embed, "challenge")
    
    # 4. ビジネストレンド速報（週2回：火曜・金曜の朝8時）
    @tasks.loop(time=datetime.time(hour=8, minute=0, tzinfo=datetime.timezone(datetime.timedelta(hours=9))))
    async def business_trends(self):
        """ビジネストレンド速報を週2回配信"""
        # 火曜日(1)と金曜日(4)のみ実行
        ...
        await self.send_to_all_guilds(embed, "trends")
    
    async def send_to_all_guilds(self, embed, content_type: str):
        """全ギルドの適切なチャンネルにメッセージを送信"""
        for guild in self.bot.guilds:
            try:
                settings = self.settings_manager.get_guild_settings(str(guild.id))
                
                # コンテンツタイプごとの有効性チェック
                if content_type == "quotes" and not settings.quotes_enabled:
                    continue
                elif content_type == "tips" and not settings.tips_enabled:
                    continue
                elif content_type == "challenge" and not settings.challenge_enabled:
                    continue
                elif content_type == "trends" and not settings.trends_enabled:
                    continue
    ...
    async def test_daily_content(self, interaction: discord.Interaction, content_type: str):
        ...
        if content_type in ['quote', '格言', '名言']:
            await self.entrepreneur_quotes()
